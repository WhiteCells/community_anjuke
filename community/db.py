...
DATABASE_URL = (
    f"mysql+pymysql://{db_config.get('user')}:{db_config.get('password')}@{db_config.get('host')}"
    f"/{db_config.get('database')}?charset={db_config.get('charset')}"
)
...
def parse_detail_page(html_path):
    ...
    with open("parsed_community.txt", "a", encoding="utf-8") as parsed_file:
        ...
        for file in os.listdir(html_path):
            ...
            try:
                with open(
                    os.path.join(html_path, file), "r", encoding="utf-8"
                ) as html_file:
                    html = html_file.read()
